...
class HomeController extends Controller {
  ...
  async getListById() {
    const id = this.ctx.params.id;
    const sql = 'SELECT article.id as id, article.title as title, article.introduce as introduce, FROM_UNIXTIME(article.addTime, "%Y-%m-%d %H:%i:%s") as addTime, article.view_count as view_count, ' +
    'type.typeName as typeName FROM article LEFT JOIN type ON article.type_id = type.id WHERE type_id = ' + id;
    const result = await this.app.mysql.query(sql);
    this.ctx.body = { data: result };
  }
